...
export interface IScraperOptions {
  ...
  jobLinksQueue: string;
  jobBoardName: string;
  concurrent?: number;
}

export interface IParsingFunctions {
  getJobInformation(link: string, html: NodeListOf<Element>): IJobInfo;
  getAllJobPageLinks(searchParams: IJobSearchOptions): Promise<string[]>;
}

export interface IFormatter {
  formatToJobInfoTableStructure(job: IJobInfo): any;
  formatToJobRequirementsStructure(job: IJobInfo): { id: string; tasks: string[] };
}

export interface IScraperConfigurationOptions {
  formatters: IFormatter;
  options: IScraperOptions;
  parsingFunctions: IParsingFunctions;
}
...
export interface JobInfoGetterNativelyFn {
  (link: string, pageContent: NodeListOf<Element>): IJobInfo;
}
